...
class LogPolar(torch.nn.Module):
    ...
    def forward(self, data):
        input_shape_x, input_shape_y = data.shape[-2], data.shape[-1]
        center_x, center_y = input_shape_x / 2, input_shape_y/2
        MAX_R = torch.log(torch.tensor(data.shape).float().norm() / 2)
        ...
        theta = theta.float()
        r = r.float()
        X = center_x + (torch.exp(r * MAX_R / self.output_shape[1])) * torch.cos(theta * 2 * torch.pi / self.output_shape[0])
        Y = center_y - (torch.exp(r * MAX_R / self.output_shape[1])) * torch.sin(theta * 2 * torch.pi / self.output_shape[0])
