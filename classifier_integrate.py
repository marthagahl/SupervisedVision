...
from dataset_sal import TransformedData
...
def largeArchitecture(num_classes, out_shape, salience, **kwargs):
...
def train(args, model, device, train_loader, optimizer, epoch, final_layer = None):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if final_layer is not None:
            output_chunks = torch.split(output, num_saliency_pts, dim=0)
            final_layer_input = torch.stack([torch.mean(o, dim=0) for o in output_chunks], dim=0)
            output = final_layer(final_layer_input)
            new_targets = torch.index_select(target, dim=0, index=torch.arange(0, len(target), num_saliency_pts).to(device))
            target = new_targets
...
def test(args, model, device, test_loader, num_classes, final_layer = None):
    model.eval()
    if final_layer is not None:
        final_layer.eval()
    test_loss = 0
...
#    if final_layer is not None:
#        return test_loss, correct * 1.0 / (len(test_loader.dataset) / salience_points) 
#    else:
...
#    test_dataset_2 = CelebADataset('/datasets01/CelebA/CelebA/072017/img_align_celeba', 'test', 'identity', ['>30'])
    ...
    model = largeArchitecture(num_classes, out_shape, args.salience)
    model = nn.DataParallel(model).cuda()
    if args.salience:
        final_layer = nn.Linear(40 * 1, num_classes)
        final_layer = nn.DataParallel(final_layer).cuda()
    ...
    if args.salience:
        optimizer =  optim.Adam(list(model.parameters()) + list(final_layer.parameters()), lr = args.initial_lr, weight_decay = 1e-3)
    else:
        optimizer =  optim.Adam(list(model.parameters()), lr = args.initial_lr, weight_decay = 1e-3)
        final_layer = None

    for epoch in range(0, args.epochs):
        train(args, model, device, train_loader, optimizer, epoch, final_layer)
        
        print("\nEvaluating on training set...")
        train_loss, train_acc = test(args, model, device, train_loader_acc, num_classes, final_layer)
        print("\nEvaluating on upright validation set...")
        val_upright_loss, val_upright_acc = test(args, model, device, val_loader_1, num_classes, final_layer)
        print("\nEvaluating on inverted validation set...")
        val_inverted_loss, val_inverted_acc = test(args, model, device, val_loader_2, num_classes, final_layer)
    ...
    test_upright_loss, test_upright_acc = test(args, model, device, test_loader_1, num_classes, final_layer)
    print("\nEvaluating on inverted test set...")
    test_inverted_loss, test_inverted_acc = test(args, model, device, test_loader_2, num_classes, final_layer)
